...
describe('notifyScheduleAssigned', () => {
  it('新建：對應到帳號就送一則 inapp，收件人帶 username（平台以 username 辨識跨系統身分）', async () => {
    const deliver = vi.fn(async () => ({ id: 'd', deduped: false, dropped: false }))
    await notifyScheduleAssigned({ schedule: sched }, { deliver, findAccount: async () => ({ username: 'rock.cai' }) })
    expect(deliver).toHaveBeenCalledWith(expect.objectContaining({ key: 'schedule_assigned:s1:2026-09-21T00:00:00.000Z', channels: ['inapp'], recipients: [{ username: 'rock.cai' }], title: '你有一筆排程：PDN-1', body: '2026/10/01 ～ 2026/10/03', linkUrl: '/vsms/' }))
  })
  it('更新但人與日期都沒變 → 不送；日期變 → 送；找不到帳號 → 不送；deliver 丟例外 → 吞掉', async () => {
    const deliver = vi.fn(async () => ({ id: 'd', deduped: false, dropped: false }))
    const findAccount = async () => ({ username: 'rock.cai' })
