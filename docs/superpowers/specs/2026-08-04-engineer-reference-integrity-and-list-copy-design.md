...
列表模式的控制列增加「複製表格」按鈕，將**當前 `filtered` 陣列的全部資料**（非可視列）同時以 TSV（`text/plain`）與 HTML `<table>`（`text/html`）兩種格式寫入剪貼簿。

- 欄位與順序同列表顯示的十欄（不含操作欄），TSV 與 HTML 兩種格式的標題列文字、欄位順序完全一致——它們是同一份資料的兩種表示法，不應各自維護而分岔
- 第一列／`<thead>` 為標題列
- TSV：分隔字元為 tab，換行為 `\n`；欄位內的 tab 與換行替換為空白，避免破壞表格結構
- HTML：`<table>` 內含 `<thead>`／`<tbody>`，僅套用最小行內樣式（collapse 邊框、儲存格留白），不重現 App 介面顏色；儲存格中的 `&`、`<`、`>` 一律跳脫，避免自由輸入文字（尤其 `taskDescription`）內的符號破壞表格結構或被瀏覽器當標籤解讀
- 狀態欄輸出狀態文字（不含符號），日期原樣輸出
- 使用 `navigator.clipboard.write` 搭配 `ClipboardItem` 同時放上兩種 MIME 類型；`ClipboardItem`／`clipboard.write` 並非所有環境都有，且 `write` 可能在 `writeText` 會成功的情況下 reject，因此失敗時退回只寫 TSV 的 `navigator.clipboard.writeText`（舊行為），兩者皆失敗才顯示提示而非無聲失敗
- 複製成功後短暫顯示已複製幾筆，讓使用者確認拿到的是完整結果而非畫面上那幾列

**為什麼要雙格式：** 純文字 TSV 只有 Excel、Google 試算表這類會把 tab 解析成欄位邊界的應用程式才能還原成表格；貼到 Word、Outlook、Google Docs 等文書處理器時，tab 不會被理解為欄位分隔，貼上的只是帶著 tab 字元的一整行純文字，不會出現表格。因此改用 `navigator.clipboard.write()` 同時放上 `text/plain`（TSV）與 `text/html`（真正的 `<table>`）兩種表示法，讓貼上的應用程式各自挑選自己看得懂的格式：試算表類應用程式仍取用 TSV，文書處理器則取用 HTML 表格。不新增相依套件。
...
| 剪貼簿表格轉換（TSV／HTML） | 兩種格式欄位順序與標題正確且彼此一致；TSV 含 tab／換行的欄位不破壞結構；HTML 的 `&`／`<`／`>` 正確跳脫、含 tab／換行的欄位不破壞列結構；空清單不產生只有標題的誤導輸出 |
